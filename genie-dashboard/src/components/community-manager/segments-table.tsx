...
const PLATFORM_CONFIG: Record<string, { color: string; label: string }> = {
  ...
  instagram: { color: "bg-gradient-to-r from-purple-500 via-pink-500 to-orange-500", label: "Instagram" },
};
...
export function SegmentsTable({
  ...
  const handlePlayClip = useCallback((segment: Segment) => {
    if (!segment.clipUrl) {
          ...
          <TableHeader>
            <TableRow className="bg-muted/50 hover:bg-muted/50">
              <TableHead className="w-10 px-3">
                <Checkbox
                  checked={
                    selectedSegmentIds.length === paginatedSegments.length &&
          ...
          <TableBody>
            {paginatedSegments.map((segment) => {
              ...
              return (
                <TableRow
                  key={segment.id}
                  className={`${isSharedRow ? "bg-green-500/5" : ""} ${isPlaying ? "bg-primary/5" : ""}`}
                >
                  {/* Checkbox */}
                  ...
                  <TableCell className="px-3">
                    <span className="text-sm">
                      {format(new Date(segment.fromTime), "MMM dd, yyyy HH:mm")}
                    </span>
                  </TableCell>
                  ...
                  <TableCell className="px-3">
                    <span className="text-sm">
                      {format(new Date(segment.toTime), "MMM dd, yyyy HH:mm")}
                    </span>
                  </TableCell>
                  ...
                  <TableCell className="px-3">
                    {processing ? (
                      ...
                      <Badge
                        variant="outline"
                        className={CATEGORY_COLORS[segment.segmentCategory] || ""}
                      >
                        {segment.segmentCategory}
                      </Badge>
                    ) : (
                          ...
                          return (
                            <Tooltip key={platform}>
                              <TooltipTrigger
                                render={
                                  <div
                                    className={`size-3 rounded-full ${config.color} cursor-help`}
                                  />
                                }
                              />
                  ...
                  <TableCell className="px-3">
                    <div className="flex items-center gap-0.5">
                      ...
                      <Tooltip>
                        ...
                        />
                        <TooltipContent>
                          {segment.clipUrl ? "Play clip" : "No clip available"}
                        </TooltipContent>
                      </Tooltip>
                      ...
                      <Tooltip>
                        ...
                        />
                        <TooltipContent>
                          {segment.clipUrl ? "Regenerate AI analysis" : "No clip to regenerate"}
                        </TooltipContent>
                      </Tooltip>
                      ...
                      <Tooltip>
                        ...
                        />
                        <TooltipContent>
                          {segment.agentResponse ? "Share to platforms" : "No post to share"}
                        </TooltipContent>
                      </Tooltip>
        ...
        <div className="flex items-center justify-between px-4 pt-4">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <span>
              ...
              {Math.min((page + 1) * rowsPerPage, sortedSegments.length)} of{" "}
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <Select
                ...
                onValueChange={(v) => setRowsPerPage(parseInt(v ?? "10", 10))}
              >
